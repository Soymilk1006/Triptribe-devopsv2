...
resource "aws_ecr_repository" "jenkins_agents" {
  name                 = var.jenkins_ecr_agent_repository_name
  image_tag_mutability = "MUTABLE"
  force_delete         = true
  image_scanning_configuration {
    scan_on_push = true
...
data "template_file" "jenkins_configuration_def" {
  ...
  template = file("${path.module}/docker/master/files/jenkins.yaml.tpl")
...
resource "null_resource" "render_template" {
  triggers = {
    src_hash = file("${path.module}/docker/master/files/jenkins.yaml.tpl")
  }
  depends_on = [data.template_file.jenkins_configuration_def]

  provisioner "local-exec" {
    command = <<EOF
tee ${path.module}/docker/master/files/jenkins.yaml <<ENDF
...
docker buildx build --platform=linux/amd64 -t ${aws_ecr_repository.jenkins_controller.repository_url}:latest ${path.module}/docker/master/ && \
...
#  provisioner "local-exec" {
#    command = <<EOF
#docker buildx build --platform=linux/amd64 -t ${aws_ecr_repository.jenkins_agents.repository_url}:latest ${path.module}/docker/agents/ && \
#docker push ${aws_ecr_repository.jenkins_agents.repository_url}:latest
#EOF
#  }
#}
